...
for date, time, and number inputs. "Locale" Support is abstracted into localizers,
making it easy to use inputs with which ever date or i18n library you choose.

## Messages

Most inputs include some amount natural language: button text, ARIA-labels, etc.
Each string is customizable per component with their `messages` prop. They can also me
configured for all components via the `Localization` Provider, rendered near your app's
root.
