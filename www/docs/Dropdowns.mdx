...
Each dropdown component accepts accept a `data` prop for providing the dropdown options.
...
function useSearch() {
  ...
  function search(term) {
    ...
    const timer = setTimeout(async () => {
      ...
      setResult(
        json.items
          ? json.items.slice(10).map((d) => d.name)
          : []
      );
      setLoading(false);
    }, 100);
...
function Example() {
  const [repos, { loading, search }] = useSearch();
