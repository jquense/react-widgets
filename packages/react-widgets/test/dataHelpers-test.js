...
describe('when using DATA HELPERS', function() {
  ...
  describe('Reading Data Text', () => {
    it('should always return a string', function() {
      expect(helpers.dataText('hi'))
        .to.be.a('string')
        .and.to.equal('hi')
      expect(helpers.dataText({ a: 3 }))
        .to.be.a('string')
        .and.to.equal('[object Object]')
      expect(helpers.dataText(null))
        .to.be.a('string')
        .and.to.equal('')
      expect(helpers.dataText(4))
        .to.be.a('string')
        .and.to.equal('4')
    })
    ...
    it('should use specified field', function() {
      ...
      expect(
        helpers.dataText(Object.create({ text: 'hi' }, {}), 'text'),
      ).to.equal('hi')
      expect(
        helpers.dataText(
          Object.create(null, { text: { value: 'hi' } }),
          'text',
        ),
      ).to.equal('hi')
      expect(helpers.dataText({ text: 'hi' }, 'text')).to.equal('hi')
      expect(helpers.dataText({ text: { a: 3 } }, 'text')).to.eql(
        '[object Object]',
      )
    })
    ...
    it('should fall back to item when missing field', function() {
      expect(helpers.dataText('hi', 'text')).to.equal('hi')
      expect(helpers.dataText({ missing: 'hi' }, 'text')).to.equal(
        '[object Object]',
      )
    })
    ...
    it('should work as a function accessor', function() {
      ...
      expect(helpers.dataText({ text: { a: 3 } }, textField)).to.eql(
        '[object Object] hi',
      )
    })
  })
  ...
  it('should work with indexOf', function() {
    ...
    const inner = { a: 2 }
    expect(
      helpers.dataIndexOf([{}, {}, { value: inner }], inner, 'value'),
    ).to.equal(2)
  })
})
