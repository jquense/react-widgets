...
export function normalizeComponent(Component) {
  return itemProps =>
    Component ? <Component {...itemProps} /> : itemProps.text || itemProps.item
}
...
export default function listDataManager(
  component,
  {
    getDataState,
  ...
  if (getDataState) getStateGetterFromProps = null
  else {
  ...
  spyOnComponent(component, {
    componentWillReceiveProps(nextProps) {
      ...
      if (needsUpdate && getStateGetterFromProps) {
        getDataState =
          getStateGetterFromProps(currentProps) || defaultGetDataState
      }
    },
  ...
  function isDisabled(item) {
    ...
    return disabled.some(
      disabled => accessors.value(item) === accessors.value(disabled)
    )
  }
  ...
  let manager = {
    ...
    prev(item, word) {
      ...
      while (
        nextIdx > -1 &&
        (isDisabled(data[nextIdx]) || !matches(data[nextIdx]))
      )
        nextIdx--
