...
function pickElementProps<T extends Record<string, unknown>>(
  props: T,
): Partial<T> {
...
const List: List = React.forwardRef(function ListImpl<TDataItem>(
  {
    multiple = false,
    data = [],
  ...
  const items = groupedData
    ? groupedData.map(([group, groupItems], idx) => (
